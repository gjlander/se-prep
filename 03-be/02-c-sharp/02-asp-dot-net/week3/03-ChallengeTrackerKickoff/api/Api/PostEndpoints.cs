...
public static class PostEndpoints
{
    public static void MapPostEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/", async (IPostService postService) =>
        {
            ...
            var postDtos = posts.Select(p => new PostResponseDto(p.Id, p.UserId, p.Title, p.Content, p.Image, p.PublishedAt));
        ...
        group.MapGet("/{id:guid}", async (Guid id, IPostService postService) =>
        {
            ...
            var postDto = new PostResponseDto(post.Id, post.UserId, post.Title, post.Content, post.Image, post.PublishedAt);
        ...
        group.MapPost("/", [Authorize] async (CreatePostDto createPostDto, IPostService postService, HttpContext context, ClaimsPrincipal user) =>
         {
             try
             {
                 ...
                 var post = await postService.CreateAsync(userId, createPostDto.Title, createPostDto.Content, createPostDto.Image);
                 var postDto = new PostResponseDto(post.Id, post.UserId, post.Title, post.Content, post.Image, post.PublishedAt);
        ...
        {
            var post = await postService.UpdateAsync(id, updatePostDto.Title, updatePostDto.Content, updatePostDto.Image);
            if (post == null)
                return Results.NotFound();
