...
## Stricter CORS considerations (move to travel journal repo)

- We can include a wildcard for our CORS policy, but if we're not planning on having other clients consume our API, we can add specific origins
...
- Make sure to update your API with these changes, to have access to the exposed headers for auto-refresh. This will also then only work on port 5173. Now if we test our request, it works

### Send Authorization header

- We get the token from local storage and add it to our headers
...
- Since we're working with access and refresh tokens, we have to get more specific
...
- We then have to re-send the request now that the token has been refreshed
  - We have to manage our tokens in local storage
...
- We need to hit the `logout` endpoint to delete refresh tokens
  - we include the refreshToken in the body, and make a POST request
...
const EditModal = ({ id, content, image, title, author }: EditModalProps) => {
